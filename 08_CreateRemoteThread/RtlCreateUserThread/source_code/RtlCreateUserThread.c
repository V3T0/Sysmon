...
BOOL RtlCreateUserThreadInProcess(HANDLE hProcess, LPWSTR DllPath, LPWSTR lpProcessName) {
	...
	LPVOID lpRtlCreateUserThread = GetProcAddress(lpNtdllHandle, "RtlCreateUserThread");
	if (lpRtlCreateUserThread == NULL) {
		printf("[ERROR] Unable to get address of RtlCreateUserThread in ntdll.dll. Exiting!");
		return FALSE;
	}
	pfnRtlCreateUserThread = lpRtlCreateUserThread;
	pfnRtlCreateUserThread(hProcess, NULL, 0, 0, 0, 0, lpLoadLibraryW, lpMemoryAddress, &hThread, NULL);
	if (hThread == NULL) {
...
int wmain(int argc, wchar_t* argv[]) {
	...
	if (!RtlCreateUserThreadInProcess(hProcess, argv[2], argv[1])) {
		return -1;
	}
}